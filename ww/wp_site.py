...
#
# description:      A class to manage WordPress websites; extends Site.
#


from ext_pylib.prompt import prompt, prompt_str, warn_prompt
from site import Site, localhost
import MySQLdb
import os
...
class WP_Site(Site):
    ...
    def __init__(self, domain, htdocs, mysql={}, is_new_site=False):
        """Initializes a new WP_Site instance."""
        ...
        else: # Current site; use wp_config
    ...
    def __str__(self):
        ...
        string = '\n----------------------------------------------------------'
        string += '\n                   - Wordpress Site -'
        string += super(WP_Site, self).__str__()
    ...
    def uninstall(self, no_prompt=False):
        ...
        if no_prompt or prompt('Remove WordPress site database "' + self.db['name'] + '"?'):
            self.remove_wordpress_db()
        if no_prompt or prompt('Remove WordPress site directory "' + self.htdocs + '"?'):
            self.remove_wordpress_files()
        super(WP_Site, self).uninstall()

    def verify(self):
        """cle"""
        super(WP_Site, self).verify()
    ...
    def remove_wordpress_files(self):
        """Removes WordPress site files."""
        print 'Removing Wordpress site files...'
        # TODO: error handling
        rmtree(self.htdocs)
        print 'Site files removed.'
    ...
    def wordpress_install(self):
        ...
        user = prompt_str('What is the WordPress admin username?', 'mm_admin')
        password = prompt_str('What is the password for this account?', 'zX8^M-byY7F%4TUH')
        email = prompt_str('What is the email for this account?', 'hbradley@mediamarketers.com')
        title = raw_input("What is the site's title? ")
